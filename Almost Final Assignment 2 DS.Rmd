...
#Question 4
```{r Q-4}
data2 <-read.csv("https://raw.githubusercontent.com/guru99-edu/R-Programming/master/adult.csv")[,-1]
data <- data2
```
...
#4.l.
We think we should remove the "education_merged"(factiroal education), because it is correlated with the "educational.num" and so less infomative. In addition we noticed that a model without it decreases the AIC and the accuracy by a bit.

We can not make any new non-linear features on the numeric values because they are standardize so log or squear on the feature is not an option.


Therefor, we will try to create the following variable as interation:


```{r Q-4-l}
glm.q4_try <- glm(income ~ age_standardize + educational.num_standardize + hours.per.week_standardize + race_merged + workclass_merged + marital.status_merged + gender + I(age_standardize^2) + I(educational.num_standardize^2) + I(hours.per.week_standardize^2) + age_standardize*race_merged + age_standardize*gender + hours.per.week_standardize*marital.status_merged + hours.per.week_standardize*gender + age_standardize*hours.per.week_standardize + educational.num_standardize*race_merged*marital.status_merged + educational.num_standardize*race_merged*gender + age_standardize*race_merged*gender + hours.per.week_standardize*race_merged*marital.status_merged)
